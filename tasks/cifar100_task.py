...
class Cifar100Task(Task):
    ...
    def load_data(self):
        ...
        if self.params.load_indices:
            indices_per_participant = np.load('./load_indices/CIFAR100_indices_per_participant.npy', allow_pickle=True).item()
            ...
            np.save('./load_indices/CIFAR100_indices_per_participant', indices_per_participant)
            train_loaders = [self.get_train(indices) for pos, indices in
                             indices_per_participant.items()]
        else:
