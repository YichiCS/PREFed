...
class Task:
    ...
    def resume_model(self):
        if self.params.resume_model:
            ...
            try:
                ...
                if self.params.task == 'Imagenet' :
                    loaded_params['fc.weight'] = torch.nn.init.xavier_uniform_(torch.empty(200, 512))
                    loaded_params['fc.bias'] = torch.nn.init.xavier_uniform_(torch.empty(200,1)).view(-1)
                    
                elif self.params.task == 'GTSRB':
                    loaded_params['fc.weight'] = torch.nn.init.xavier_uniform_(torch.empty(43, 512))
                    loaded_params['fc.bias'] = torch.nn.init.xavier_uniform_(torch.empty(43,1)).view(-1)
    ...
    def report_metrics(self, step, prefix='', index=None):
        metric_text = []
        for metric in self.metrics:
            metric_text.append(str(metric))
        if index is not None:
            logger.warning(f'{prefix} {index} {step:4d}. {" | ".join(metric_text)}')
        else:
