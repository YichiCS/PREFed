...
class PatternSynthesizer(Synthesizer):
    trigger: torch.Tensor = torch.tensor([
        [1., 0., 1.],
        [-10., 1., -10.],
        [-10., -10., 0.],
        [-10., 1., -10.],
    ...
    def __init__(self, task: Task):
        super().__init__(task)
        self.make_pattern(self.trigger, self.x_top, self.y_top)

    def make_pattern(self, trigger, x_top, y_top):
        full_image = torch.zeros(self.params.input_shape)
        full_image.fill_(self.mask_value)

        x_bot = x_top + trigger.shape[0]
        y_bot = y_top + trigger.shape[1]
        ...
        full_image[:, x_top:x_bot, y_top:y_bot] = trigger
